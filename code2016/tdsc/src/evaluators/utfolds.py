...
class JphKfold(KFold):
    def __init__(self,k,data_set,metrics, friends_data=None,friends_num=10): 
        super(JphKfold,self).__init__(k,data_set)
        self.metrics = metrics
        self.friends_data = friends_data
        self.friends_num = friends_num
        return

    def validate(self,train_set,test_set):
        valuator = JphEvaluator(train_set, test_set, self.friends_data, self.friends_num)
        return valuator.evaluate(self.metrics)


class EsoricsSingleUserValidation(KFold):
    def __init__(self,k,data_set,friends_data, f_n, t_n, f_ratio, testfriend=True):
        super(EsoricsSingleUserValidation,self).__init__(k,data_set)
        self.friends_data = friends_data
        self.f_n = f_n
        self.t_n = t_n
        self.f_ratio = f_ratio
        self.testfriend = testfriend
        return
    ...
    def cross_validate(self):
        ...
        for oth in range(0,50):  #spcial test trick, 50 times 5-fold cross validation
            print "start cross validation >  ", oth, "/50 run..."
        ...
        #print " additional information for exp interest :=> ", tt_idx, " ", ss_idx, " ", ssum_idx
        return xresults


    def validate(self,train_set,test_set):
        valuator = EsoricsSingleUserInfluenceEvaluator(train_set, test_set, self.friends_data, self.f_n, self.t_n, self.f_ratio,self.testfriend)
