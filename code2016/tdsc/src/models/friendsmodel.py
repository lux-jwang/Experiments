...
class FriendsModel(Model):
    ...
    def get_friends(self,user_id, level=1, incl1st=1):
        if not user_id in self.friends_data:
            return None
        if level == 1:
            return self.friends_data[user_id]
        if level == 2:
            if user_id in self.friends_upd:
                return self.friends_upd[user_id]
            indx = 0
            scnd_friends = []
            ...
            f_list = list(f_set)
            self.friends_upd[user_id] = f_list
    ...
    def is_a_friend(self,user_id, f_id,level=1):
        ...
        #SHOULD BE THIS?
        tmp_f_list = self.get_friends(user_id,level)
        if not tmp_f_list:
            self.friends_cache[user_id][f_id] = False
            return False

        if f_id in tmp_f_list:            
