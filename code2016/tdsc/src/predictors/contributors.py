...
class Friends_Strangers(Contributors):
    ...
    def get_rand_friends(self, user_id):
        ...
        f_ids = self.model.get_friends(user_id,level=1)
        shuffle(f_ids)
        #rint "f_ids: ", len(f_ids)
        frs = f_ids[:self.f_n]
        fnbs = self.similarity.get_similarities(user_id,frs)
        #fnbs = fnbs[:self.f_n]
        #print "fnbs: ", len(fnbs)
        self.friends[user_id] = fnbs;
        return fnbs
    ...
    def get_rand_strangers(self, user_id):
        ...
        shuffle(t_ids)
        sts = t_ids[:self.t_n]
        st_sim = [0.5]*len(sts) #specific sim for esorics prefictor
        stnbs = zip(sts,st_sim)

        self.strangers[user_id] = stnbs;

        return stnbs

    def get_fof_as_strangers(self,user_id):
        if user_id in self.strangers:
            return self.strangers[user_id]
        t_ids = self.model.get_friends(user_id,level=2,incl1st=None)
        shuffle(t_ids)
        sts = t_ids[:self.t_n]
        st_sim = [0.5]*len(sts) #specific sim for esorics prefictor
        stnbs = zip(sts,st_sim)
