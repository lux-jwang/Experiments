...
void perform_topn_stranger(CSTMARK int *pRt, BIGPOLYARRAY *pIb, BIGPOLYARRAY *pBias, int item_size)
{
    ...
    //cout <<"one user: " << setprecision(5)<<float(clock ()-begin_time )/CLOCKS_PER_SEC <<" seconds" <<endl;	
}
...
void perform_topn(UID u_id, int f_num=70, int t_num=10, int item_size=1682)
{
    ...
    {
        pQTs[indx] = new BIGPOLYARRAY[item_size];
        pBATs[indx] = new BIGPOLYARRAY[item_size];

    	perform_topn_stranger(pTdata[indx], pBATs[indx], pQTs[indx], item_size);
    }
    ...
    {
        pQFs[indx] = new BIGPOLYARRAY[item_size];
        pBAFs[indx]= new BIGPOLYARRAY[item_size];
    	perform_topn_friend(pFdata[indx],pUavec[indx],pBAFs[indx],pQFs[indx],item_size);
    }
